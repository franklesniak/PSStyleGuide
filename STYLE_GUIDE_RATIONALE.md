...
The prefix list in [Local Variable Naming: Type-Prefixed camelCase](STYLE_GUIDE.md#local-variable-naming-type-prefixed-camelcase) is deliberately closed with respect to the common built-in types it enumerates: `$obj` is already established as the default prefix for any .NET type without a dedicated approved prefix (including enum values), and `$hashtable` is already established as the canonical prefix for hashtable variables. When authors invent parallel abbreviated prefixes such as `$enum…` for enums or `$hash…` for hashtables, they fragment the convention: reviewers scanning code can no longer rely on a single, predictable token to identify a variable's type, and searches for `$hashtable` (or for `$obj` on enums) miss the ad hoc variants.

Two specific substitutions are called out because they are the most tempting mistakes:

- **`$enum…` → `$obj…`.** An enum is a .NET reference type without a dedicated approved prefix, so it falls under the default `$obj` bucket exactly like any other non-primitive type. Introducing a separate `$enum` prefix would imply that enums are a first-class category in the prefix list, when in fact the guide's design is for `$obj` to absorb all such types uniformly.
