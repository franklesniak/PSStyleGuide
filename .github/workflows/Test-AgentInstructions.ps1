...
if ($SelfTest) {
    ...
    if ([regex]::Matches(
            $strValidatorSource,
            '(?m)^# Version: 1\.9\.20260913\.4$'
        ).Count -ne 1) {
        throw 'The validator script version is not 1.9.20260913.4.'
    }
    ...
    if ([regex]::Matches(
            $strTrustRootAuthorizationSource,
            '(?m)^# Version: 1\.2\.20260913\.3$'
        ).Count -ne 1) {
        throw 'The trust-root authorization script lacks version 1.2.20260913.3.'
    }
    & (Join-Path $strRepositoryRootPath $strTrustRootAuthorizationPath) `
        -RepositoryRootPath $strRepositoryRootPath `
