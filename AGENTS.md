...
5. If the supported interface cannot select `Balanced`, record the reason and use `gh pr edit PR-NUMBER --add-reviewer @copilot`. If that command is unavailable, use `gh api --method POST "repos/OWNER/REPOSITORY/pulls/PR-NUMBER/requested_reviewers" -f "reviewers[]=copilot-pull-request-reviewer[bot]"`. A resulting `Lite` review is an acceptable fallback. It does not fail or stall the review loop.
