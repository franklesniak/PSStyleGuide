...
- **Use `[ValidateRange(min, max)]` on numeric parameters when:**
  - The parameter has a bounded valid domain, such as counts, delays or timeouts, thresholds, or percentages.
  - Fail-fast parameter binding provides a clearer error than allowing an invalid value to surface later in downstream logic.
  - Prefer explicit caps when the domain is naturally bounded (for example `0, 100` for percentages); when no principled upper bound exists, a type maximum such as `[int]::MaxValue` or `[double]::MaxValue` **SHOULD** be used.
