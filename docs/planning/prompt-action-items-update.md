...
The compact resume state must retain only immutable predecessor outputs that a later task still needs. Key each value by producing task and exact output name, record its final consumer, and prune it after that consumer completes. During ingestion, require the producer to be complete and require the final consumer to remain incomplete. Before JSON parsing can change a value, reject a numeric token if it is non-finite, its absolute value exceeds 9007199254740991, it is negative zero, or conversion to an ECMAScript `Number` changes its exact decimal value. Store an exact value as a string if its numeric token fails any condition. When review state exists, require the current task head to equal the review-input head. Use closed bounded maps and reject duplicate JSON members during ingestion.
...
2. one Codex and one Copilot review for the frozen reviewed input by default, with at most two automatic same-channel retries after exact attributable downstream terminal failures;
...
5. merge only after both gates apply to the same final head, tree, and frozen semantics, except when one narrow typed explicit-operator exhaustion authority permits exhausted-not-clean readiness and every other gate still passes;
...
Serialize pairs for different reviewed inputs. If authenticated reviewed-input drift makes an unrequested old-input channel impossible, store one typed `SUPERSEDED` disposition only after every recorded request for the old input is terminal. Cross-validate that disposition against request existence, an incomplete channel set, one matching head, a known successor head, and a time not earlier than every described request terminal-result or terminal-disposition boundary and not later than the successor request; require the disposition before a different-input successor request and do not create synthetic evidence. Require a submitted review's commit to match the reviewed head. Attribute a headless Codex PR-conversation result only through authenticated author, request time, exclusion of every supplied identity from the baseline, mutually consistent valid timestamp aliases, required matching normalized head evidence when terminal, reviewed-input key, and serialized predecessor-pair order. Store conversation baselines as a node-ID-to-timestamp map.
...
For each confirmed terminal request, persist exactly one closed outcome reference while the complete result stays in its separate channel result collection. Use one closed `terminalResultRef` only for a clean result. Use one closed `terminalFailureRef` for an exact terminal non-success, including its immutable failure-detail identity and mutable summary observation. During compact-state ingestion, cross-validate the outcome against the exact attempt, channel, actor, every supplied request-baseline identity, losslessly ordered mutually consistent timestamp aliases, required matching normalized head evidence, reviewed head, next same-channel request boundary, and next different-input request boundary. Reject a missing, duplicate, stale, baseline, wrong-actor, wrong-head, wrong-channel, wrong-attempt, or wrong-time reference. Do not add either reference to an unconfirmed or nonterminal request.
