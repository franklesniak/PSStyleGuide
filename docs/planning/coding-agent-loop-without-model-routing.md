...
Execute once. Validate the native response and perform one targeted authenticated readback. Do not retry an ambiguous non-idempotent operation. After a successful response and matching readback, a local serialization failure cannot authorize a repeated public mutation.
