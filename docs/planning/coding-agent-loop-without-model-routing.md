...
An exhausted reviewer channel is blocked by default. An exception requires one closed `reviewerExhaustionAuthority` whose state is `OPERATOR_AUTHORIZED_EXHAUSTED_NOT_CLEAN` and that binds the explicit operator decision to the exact repository, pull request, reviewed-input key, head, Codex channel, maximum attempt 3, more than 20 completed review rounds, authorization time, authority, and reason. Report an effective exception as `EXHAUSTED_NOT_CLEAN`. It can release the independent quality task, but it cannot mark Codex clean. It can make merge readiness true only when exact-head CI, the exact-input Copilot outcome or its independently authorized nonfunctional disposition, all actionable-finding and thread resolution, the fresh independent quality audit, exact-head final validation, frozen-input accuracy, mergeability, and every other required R3 gate pass. A changed reviewed input starts a new lifecycle.
