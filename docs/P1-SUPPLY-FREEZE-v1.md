...
From the expected PS clone, run the complete block below with the verified `strNode` from above. Acquisition writes the Git object database before the read-only verification interval. It always refetches the exact historical commit from the canonical explicit URL with `--no-tags --refetch` and no filter, including when the commit already exists. Do not replace the URL with a configured filtered remote. The `&&` connects acquisition to verification: failed acquisition returns its native nonzero status and does not start the verifier or print its success marker, even when older local objects are complete. After successful acquisition, the verifier's native status is the block's status. There is no automatic local probe or conditional-fetch interface.

```bash
git fetch --no-tags --refetch https://github.com/franklesniak/PSStyleGuide.git \
...
[Issue 204](https://github.com/franklesniak/PSStyleGuide/issues/204) adapts the common corrections from Terraform commit `855580f008c8a27bc42f2cce426fc0d7459be9f5` to PS baseline `5a6f22f10dd244e5d4f0b0b9448ccaeeb18daaf0`. The [pinned comparison](https://github.com/franklesniak/PSStyleGuide/issues/159#issuecomment-5769973474) identifies the exact role blobs, all noncommon intervals, and the repository-specific identity, schema, history, provenance-object, and document-role differences. The current recorder transfers the bounded mount guard and its causal fixtures; the method and tests now state native launch exit 2 and nullable incomplete outcomes. The historical acquisition recipe uses the canonical explicit URL and no-filter refetch, gates its own verifier on native fetch success, and retains independent offline verification. Neither recipe runs Terraform's archived recorder or changes PS's historical objects or advisory assertions. The candidate validation record must bind the exact final bytes and current 16-row matrix. Landing and a later reciprocal comparison remain separate requirements; this method does not assert their completion.
