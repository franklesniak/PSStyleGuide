...
From the expected PS clone, run the complete block below with the verified `strNode` from above. Acquisition writes the Git object database before the read-only verification interval. The first Git command resolves the effective URL without contacting the remote and refuses when configuration would rewrite the canonical literal; see [`git ls-remote --get-url`](https://git-scm.com/docs/git-ls-remote#Documentation/git-ls-remote.txt---get-url). Keep repository, included, global, and system Git configuration unchanged from that check through the fetch. Concurrent same-UID configuration changes remain a caller exclusion. This check does not disable authentication, proxy, helper, redirect, or other transport behavior. Acquisition then refetches the exact historical commit from the canonical explicit URL with `--no-tags --refetch` and no filter, including when the commit already exists. Do not replace the URL with a configured filtered remote. Each `&&` stops the block after a failed or mismatched URL check or a failed acquisition, before the verifier or its success marker can run. A failed fetch returns its native nonzero status, even when older local objects are complete. After successful acquisition, the verifier's native status is the block's status. There is no automatic local object probe or conditional-fetch interface.

```bash
strCanonicalRemote='https://github.com/franklesniak/PSStyleGuide.git'
...
### Current common sync-back

[Issue 204](https://github.com/franklesniak/PSStyleGuide/issues/204) adapts the common corrections from Terraform commit `855580f008c8a27bc42f2cce426fc0d7459be9f5` to PS baseline `5a6f22f10dd244e5d4f0b0b9448ccaeeb18daaf0`. The [pinned comparison](https://github.com/franklesniak/PSStyleGuide/issues/159#issuecomment-5769973474) identifies the exact role blobs, all noncommon intervals, and the repository-specific identity, schema, history, provenance-object, and document-role differences. The current recorder transfers the bounded mount guard and its causal fixtures; the method and tests now state native launch exit 2 and nullable incomplete outcomes. Review repairs accept the documented zero mount IDs, precompute bounded topology relationships, classify coordinate failures as topology refusals, and refuse configured rewrites of the canonical acquisition URL. The historical acquisition recipe still uses the canonical explicit URL and no-filter refetch, gates its own verifier on native preflight and fetch success, and retains independent offline verification. These common parser, acquisition, and validation additions require reciprocal Terraform assessment in Task 152; they are not permanent repository-specific exceptions or a claim of full convergence. Neither recipe runs Terraform's archived recorder or changes PS's historical objects or advisory assertions. The candidate validation record must bind the exact final bytes and current 16-row matrix. Landing and a later reciprocal comparison remain separate requirements; this method does not assert their completion.

### Original-port comparison (historical)

The matrix and original follow-up allocation below retain the original port's comparison context; they do not classify the current sync-back or grant permanent exceptions. The unchanged cycle-3 foundation and its complete role map remain established by the [permanent closure](https://github.com/franklesniak/PSStyleGuide/issues/159#issuecomment-5751908399) and [full reciprocal evidence](https://github.com/franklesniak/TerraformStyleGuide/issues/31#issuecomment-5751829095). This recorder does not reopen those workflow or generator implementations. The original source was Terraform commit `aae05282b57f093cec8b63e59138db72c982f10e`; the original destination was the PS candidate based on `986a78cfad02abe9698ee258735d8451abeb9249`.

| Row | Status | Normative/implementation locator, observed behavior, and applicability proof |
| --- | --- | --- |
| GF-PARAMETERS | Intentional difference | This method's invocation contract; recorder argument/cache validation. Retains upstream diagnostic flags; adds one external housekeeping directory. Cache refusal cases apply even with bypass. Audit registry binding uses a child environment rather than a command-line argument. |
...
| GF-FAILURE | Intentional difference | Upstream refusals plus cache 16 and assertion 17; initial self-snapshot and later open/fstat/read/close failures retain caller-specific exits, audit native failures are tightened, observable unsafe startup/debug settings refuse 2, input JSON and semantic npm stdout reject invalid UTF-8 at their owning phase, bounded npm response overflow refuses at its owning phase, present non-null non-string advisory URLs reach the fixed audit-normalization refusal, and project-tree special entries, escaping/unresolved links, or npm-installation special entries remain refusals under `--any-toolchain` (installed tree exit 11, npm installation exit 2). Focused adapters prove controlled refusal outcomes; Linux whole-process cases prove no-record refusals. No rollback or atomic snapshot claim. |
...
| GF-CREDENTIALS | Same applicable restriction | No GitHub token or push credential is requested. Upstream public-registry/transport checks remain; the checked audit registry is absent from argv, case-insensitive child aliases are replaced by one canonical setting, and wrapper diagnostics publish only fixed operation labels. Unsupported argv, filesystem paths/names, malformed JSON, npm-tree diagnostics, public npm-version labels, unreviewed URL authorities/details, and successful audit display publish no raw response/path/package strings, while private npm logs and same-UID or privileged process inspection remain sensitive. Cache authority grants no repository write authority. |
| GF-EVIDENCE | Intentional difference | Focused production-function and whole-recorder tests cover argument and filesystem-diagnostic privacy, initial self-snapshot and descriptor failure mapping, internal directory control, JSON and semantic-stdout UTF-8/size response privacy, advisory URL typing, fixed audit summaries, unconditional npm identity, pre-child special refusal, link containment, non-owner write modes, Node debug isolation, npm version/URL projection, npm-tree diagnostic privacy, closed Git-child environments, and setup order; retained mutation probes, the historical verification procedure, and full before/after snapshots bind the claims. No temporary workflow or review/merge is part of this implementation task. |

Issue 158 remains nonblocking for issues 147 and 152. There is no new dependency edge, advisory extension, protected instruction change, or workflow integration. Task 134 must explicitly assess the D1-D5 port differences and the later review repairs, including contract/descriptor refusal classification, recursive contract and strict-configuration comparison refusal, the nonblocking initial self-snapshot, structured npm diagnostics, skipped-audit field presence, setup prerequisites and external-base/early-umask handling, repository-internal input-directory admission, npm-installation and unconditional installed-tree symlink containment, complete unsupported-token and filesystem-location withholding, JSON-response and semantic-stdout UTF-8 refusal, bounded npm-response phase mapping, fixed audit display summaries, advisory URL type validation, unconditional npm executable identity, non-owner write-mode refusals, Node debug isolation, npm-tree diagnostic privacy, audit-registry environment binding with operation-only diagnostics, phase-specific Node-distribution resolution refusal, and the focused-test extraction boundary. These are explicit source differences, not a claim of whole-recorder convergence or a permanent PS-only exception. This narrow matrix does not assert a new whole-repository fixed point.
