...
| GF-NODE-LOCK | Intentional difference | Existing P1 manifests, historical Windows producer, and unchanged lock. Current Linux/npm authentication retained; external cache/log routing and caller/child runtime-output isolation strengthened. Strict live success and full read-only snapshots prove current effects. |
...
| GF-EVIDENCE | Intentional difference | Focused production-function and whole-recorder tests, historical verification procedure, and full before/after snapshots. No temporary workflow or review/merge is part of this implementation task. |

Issue 158 remains nonblocking for issues 147 and 152. There is no new dependency edge, advisory extension, protected instruction change, or workflow integration. A later review task must explicitly assess these port-specific differences; this narrow matrix does not assert a new whole-repository fixed point.
