...
| 2 | Unsupported invocation, observable active startup-output/debug environment, host, toolchain, npm identity, special npm-installation entry, or subprocess launch failure. |
...
| GF-NODE-LOCK | Intentional difference | Existing P1 manifests, historical Windows producer, and unchanged lock. Current Linux/npm authentication retained; external cache/log routing, caller/child runtime-output and debug isolation, a complete project-tree fold before npm, unconditional project-tree special/link refusal, npm-installation link/special refusal, and non-owner write-mode refusal are strengthened. The setup establishes umask `0022` after nonmutating base checks and before its first artifact. Strict live success, bounded setup cases, and full read-only snapshots prove current effects. |
